...
| The Vault | `/vault` | Evidence & document system. Ingests real SEC filings from EDGAR (free, no key — 10-K/10-Q/8-K and 20-F/6-K for foreign issuers), manual transcript/note upload, Postgres full-text keyword retrieval (+ pgvector embeddings when an OpenAI key is configured), and grounded Q&A with citations. **No source, no claim.** |
