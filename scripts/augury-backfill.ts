...
//   AUGURY_BACKFILL_MAX_ITEMS    optional per-author cap (default: FULL history)
//   AUGURY_BACKFILL_WINDOW_DAYS  optional dated-window size for paging (default 90)
...
// Omitted by default → page each author's FULL history. Set to cap a run.
const MAX_ITEMS = process.env.AUGURY_BACKFILL_MAX_ITEMS
  ? Number(process.env.AUGURY_BACKFILL_MAX_ITEMS)
  : undefined;
...
async function main(): Promise<void> {
  ...
  console.log(
    `Augury backfill — ${active.length} active author(s), ${
      MAX_ITEMS != null ? `up to ${MAX_ITEMS} tweets each` : "FULL history each (windowed paging)"
    }.\n`,
  );
  ...
  console.log("New posts each enqueued a 'resolve' job; each author enqueued a 'profile' job.");
