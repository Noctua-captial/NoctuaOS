...
export default async function AuguryOverview({
  ...
  const [scoreRows, postCountRows, callCountRows, jobRows, totals, positionRows] = await Promise.all([
    authorIds.length
    ...
    authorIds.length
      ? db
          .select({
            authorId: tables.auguryPositions.authorId,
            subjectType: tables.auguryPositions.subjectType,
            status: tables.auguryPositions.status,
          })
          .from(tables.auguryPositions)
          .where(inArray(tables.auguryPositions.authorId, authorIds))
      : Promise.resolve([]),
  ]);
  ...
  // Positions/themes rollup across tracked authors (graceful when the link
  // stage hasn't run: positionRows is simply empty).
  const openByAuthor = new Map<number, number>();
  let posOpen = 0;
  let posWatching = 0;
  let posClosed = 0;
  let posTicker = 0;
  ...
  for (const p of positionRows) {
    if (p.status === "open") {
      posOpen += 1;
      openByAuthor.set(p.authorId, (openByAuthor.get(p.authorId) ?? 0) + 1);
    } else if (p.status === "watching") {
      posWatching += 1;
    } else if (p.status === "closed") {
      posClosed += 1;
    }
    if (p.subjectType === "ticker") posTicker += 1;
    else if (p.subjectType === "theme") posTheme += 1;
    else if (p.subjectType === "macro") posMacro += 1;
  }
  const totalPositions = positionRows.length;

  // Semantic memory search ("everything they ever said about X") across all
  // tracked authors. searchPosts is a sibling-WIP import; be defensive if it
  // throws (keyless embeddings, empty corpus) and degrade to no results.
  const q = (rawQ ?? "").trim();
  let searchHits: SearchHit[] = [];
  const searchHandleById = new Map<number, string>();
  if (q) {
    try {
      searchHits = normalizeSearchHits(await searchPosts(q, { limit: 24 }));
    } catch {
      searchHits = [];
    }
    const hitAuthorIds = [...new Set(searchHits.map((h) => h.authorId).filter((x): x is number => x != null))];
    if (hitAuthorIds.length) {
      const rows = await db
        .select({ id: tables.authors.id, handle: tables.authors.handle })
        .from(tables.authors)
        .where(inArray(tables.authors.id, hitAuthorIds));
      for (const r of rows) searchHandleById.set(r.id, r.handle);
    }
  }
  ...
  return (
    <div>
      ...
      <div className="px-10 pt-8">
        ...
        {/* positions / themes rollup across tracked authors */}
        <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-2 border border-line bg-ink-card px-5 py-3">
          <span className="label !text-[8px]">Positions</span>
          {totalPositions > 0 ? (
            <>
              <span className="fin text-[10px] text-parchment-faint">
                open <span className="text-bull">{posOpen}</span>
              </span>
              <span className="fin text-[10px] text-parchment-faint">
                watching <span className="text-warn">{posWatching}</span>
              </span>
              <span className="fin text-[10px] text-parchment-faint">
                closed <span className="text-parchment-dim">{posClosed}</span>
              </span>
              <span className="text-parchment-faint">·</span>
              <span className="fin text-[10px] text-parchment-faint">
                tickers <span className="text-platinum">{posTicker}</span>
              </span>
              <span className="fin text-[10px] text-parchment-faint">
                themes <span className="text-warn">{posTheme}</span>
              </span>
              <span className="fin text-[10px] text-parchment-faint">
                macro <span className="text-parchment-dim">{posMacro}</span>
              </span>
              <span className="fin ml-auto text-[10px] text-parchment-faint">
                {totalPositions} campaign{totalPositions === 1 ? "" : "s"} across {TRACKED_AUTHORS.length} authors
              </span>
            </>
          ) : (
            <span className="fin text-[10px] text-parchment-faint">
              Not yet computed — run the pipeline&apos;s link stage to thread calls into campaigns.
            </span>
          )}
        </div>
      </div>

      {/* semantic memory — everything they ever said about X */}
      <div className="px-10 pt-6">
        <div className="label mb-3">Semantic Memory — everything they ever said about…</div>
        <AugurySearchBox initialQuery={q} />
        {q && (
          <div className="mt-4">
        ...
        {!hasPosts ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            {TRACKED_AUTHORS.map((tracked) => {
              ...
              const openCount = author ? openByAuthor.get(author.id) ?? 0 : 0;
                ...
                >
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <span className="label !text-[8px]">
                      {author ? `${openCount} open · latest ${daysAgo(latest)}` : "not yet ingested"}
                    </span>
